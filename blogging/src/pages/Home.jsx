...
function Home() {
  ...
  // Open comment modal for a specific blog
  const openCommentModal = async (blogId) => {
    ...
    try {
      ...
      // Map the response comments into the structure you need
      const transformedComments = response.data.comments.map((comment) => ({
        text: comment.comment, // Assuming the comment text is stored in `comment.comment`
        user: comment.name, // Assuming the user name is stored in `comment.name`
        isUser: comment.name === userName, // If the comment's user is the logged-in user, set isUser to true
      }));
